...
# Output
print(paste("Rounded samplig error:", round(e$e, 2))) # Print the rounded sampling error
print(paste("Samplig error:", e)) # Print the rounded sampling error
```

Note that the unrounded sampling error gives a decimal number that is very close, but is not equal, to our sampling error of 0.1. This happens because the sample sizes must be whole numbers, with the exact but inapplicable sample size returned by the function being rounded up. In fact, if in the previous function we could enter the exact sample size prior to rounding, then we would obtain the exact previously defined sampling error.
