#' Sampling error when estimating the mean with a Stratified Sampling plan
#'
...
#'
#' @return The function returns the sample error consistent with the estimation of the mean of a phenomena, given the sample size.
#' @export
#'
#' @examples ex_sts(C = 0.95, n_real = c(48, 14, 7), sd_est = c(0.2, 0.5, 0.7), alloc =c(0.7, 0.2, 0.1), parameter = TRUE, N = c(1400, 400, 200))


# Sample error function for stratified sampling
ex_sts <- function(C, n_real, sd_est, alloc = NULL, parameter = FALSE, N) {
  ...
  return(list(E_min = E_min, n_real = n_real, n_adjusted = ceiling(n_adjusted * alloc), diff = sum(n_adjusted * alloc - n_real)))
}
