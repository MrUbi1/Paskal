...
#' @param m Average cluster size (number of elements), either of the population (prefered) or of a preliminar sample; m > 0.
#' @param N A positive integer indicating the population size (id est, the total number of clusters). Defaults to infinite; N > 0.
...
#' @export
#'
#' @examples np_cls(C = 0.95, e = 0.04, sd_exp = 0.726, m = 6.04, N = 415, parameter = TRUE) # Ejemplo 8.10
...
np_cls <- function(C, e, sd_exp, m, N = Inf, parameter = FALSE) {
  ...
  N <- ifelse(is.infinite(N), 1000000, N)
