#' Sample size needed to estimate the total with a Stratified Sampling plan
#'
...
#'
#' @return The function returns the sample size needed to estimate the total of a phenomena, consistent with the risk ('C' and 'e') that the auditor is willing to assume.
#' @export
#'
#' @examples nt_sts(C = 0.95, E = 200, sd_exp = c(5, 15), alloc = c(1/2, 1/2), parameter = TRUE, N = c(155, 62))
#' @examples nt_sts(C = 0.95, E = 400, sd_exp = c(5, 15, 10), alloc = c(1/2, 1/4, 1/4), parameter = FALSE, N = c(155, 62, 93))
...
nt_sts <- function(C, E, sd_exp, alloc = NULL, parameter = FALSE, N) {
